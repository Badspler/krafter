...
public class Scraper {
    ...
    private WebDriver createChromeDriver() {
        ...
        //Avoid triggering Cloudflare's automation detection
    ...
    private void writeOutput(ArrayList<AnimeSeries> series) {
    ...
    //Season chart show links don't have a class to select by, so they're addressed positionally by index
    private static final String SEASON_ARTICLE_LINK_TEMPLATE = "/html/body/div[2]/main/article[%d]/div/h3/a";
    private static final String STREAM_ITEM_LINK_TEMPLATE = "/html/body/div[1]/div[2]/div[4]/div[%d]/div/a";

    //TODO: these wait times are guesses to ride out Cloudflare's bot check - unclear what the actual minimum is
    private static final long ANIME_PAGE_LOAD_WAIT_MS = 2000;
    private static final long SEASON_CHART_LOAD_WAIT_MS = 7000;
    private static final long STREAMS_PAGE_LOAD_WAIT_MS = 4000;
    ...
    private void scrapeEachAnime(WebDriver driver, ArrayList<AnimeSeries> series, ArrayList<String> animeList) {
        ...
        for (String url : animeList) {
            ...
            try { // Bonus little wait for cloudflare
                Thread.sleep(ANIME_PAGE_LOAD_WAIT_MS);
            ...
            WebElement sourceOrOriginalElement = findElementByXpath(driver, SOURCE_OR_ORIGINAL_BY_TEXT, "SOURCE_OR_ORIGINAL_BY_TEXT");
            if (sourceOrOriginalElement != null) {
                boolean isOriginal = sourceOrOriginalElement.getText().contains("Original");
                anime.setHasSource(!isOriginal);
            }

            //find series format (TV, OVA, ONA, etc)
            WebElement formatElement = findElementByXpath(driver, FORMAT_BLOCK, "FORMAT_BLOCK");
            if (formatElement != null) {
                ...
                WebElement parentDiv = formatElement.findElement(By.xpath("./.."));
                String textValue = parentDiv.getText().replace("Format", "").replace("\n", "").trim();
                anime.setSeriesFormat(textValue);
            }

            //find external resources by class id
            anime.addInfo(url); //livechart.me
            addInfoFromButton(driver, anime, ANILIST_BUTTON, "ANILIST_BUTTON");
            addInfoFromButton(driver, anime, ANIME_PLANET_BUTTON, "ANIME_PLANET_BUTTON");
            addInfoFromButton(driver, anime, MAL_BUTTON, "MAL_BUTTON");
            addInfoFromButton(driver, anime, ANIDB_BUTTON, "ANIDB_BUTTON");
            addInfoFromButton(driver, anime, KITSU_BUTTON, "KITSU_BUTTON");
            ...
            try {
                Thread.sleep(STREAMS_PAGE_LOAD_WAIT_MS);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
    ...
    //Finds an external-site button by its xpath and, if present, adds its link to the anime's info list
    private void addInfoFromButton(WebDriver driver, AnimeSeries anime, String xpath, String elementFinding) {
        WebElement button = findElementByXpath(driver, xpath, elementFinding);
        if (button != null) {
            anime.addInfo(button.getAttribute("href"));
        }
    }
    ...
    private ArrayList<String> scrapeSeasonChart(WebDriver driver, String livechartURL) {
        ...
        try {
            ...
            Thread.sleep(SEASON_CHART_LOAD_WAIT_MS);
        ...
        int count = 1;
        List<WebElement> articleLinks = driver.findElements(By.xpath(String.format(SEASON_ARTICLE_LINK_TEMPLATE, count)));
        while (!articleLinks.isEmpty()) {
            //Don't scan anything from the ignoreList
            if (blockList.contains(articleLinks.get(0).getAttribute("href")) || blockList.contains(articleLinks.get(0).getText())) {
                System.out.println("Excluding: " + articleLinks.get(0).getText());
                count++;
                articleLinks = driver.findElements(By.xpath(String.format(SEASON_ARTICLE_LINK_TEMPLATE, count)));
                continue;
            }

            animeList.add(articleLinks.get(0).getAttribute("href"));
...
//                break;
    ...
    private AnimeSeries findStreams(WebDriver driver, AnimeSeries anime, String url) {
        ...
        int count = 1;
        WebElement streamsContainer = findElementByXpath(driver, STREAMS_CONTAINER, "STREAMS_CONTAINER");

        while (streamsContainer != null) {
            WebElement innerElement = findElementByXpath(driver, String.format(STREAM_ITEM_LINK_TEMPLATE, count), "Stream: " + count);
